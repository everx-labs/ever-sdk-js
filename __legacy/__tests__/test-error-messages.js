...
async function expectError(code: number, source: ?string, f: () => Promise<void>) {
    try {
        await f();
        // $FlowFixMe
...
test.each(ABIVersions)('Test SDK Errors 1-3 (ABI v%i)', async (abiVersion) => {
    ...
    await expectError(2, 'client', async () => {
        await contracts.createDeployMessage({
    ...
    await expectError(1009, 'client', async () => {
        await contracts.runGet({
            codeBase64: '',
...
test.each(ABIVersions)('Test SDK Errors > 2000 (ABI v%i)', async (abiVersion) => {
    ...
    await expectError(2018, 'client', async () => {
        await crypto.hdkeyXPrvDerivePath('', '', true);
    });

    await expectError(2019, 'client', async () => {
    ...
    await expectError(2022, 'client', async () => {
...
test.each(ABIVersions)('Test SDK Errors 3000-3020 (ABI v%i)', async (abiVersion) => {
    ...
    await expectError(3002, 'client', async () => {
        await contracts.createDeployMessage({
            package: {
                // $FlowFixMe
                abi: '',
    ...
    await expectError(3003, 'client', async () => {
        await contracts.createDeployMessage({
            package: {
                // $FlowFixMe
                abi: '',
    ...
    const body = '';
    await expectError(3005, 'client', async () => {
        await contracts.decodeOutputMessageBody({
            abi: walletPackage.abi,
            bodyBase64: body,
        });
    });
    await expectError(3006, 'client', async () => {
    ...
    await expectError(3018, 'client', async () => {
