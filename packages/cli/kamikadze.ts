...
export async function kamikadze(options: { value: number }) {
    ...
    try {
        ...
        const giverBalanceBefore = BigInt(
            (await giver.account.getBalance()) ?? 0,
        )
        ...
        const giverBalanceAfter = BigInt(
            (await giver.account.getBalance()) ?? 0,
        )
        console.log(
            `Giver balance: ${giverBalanceAfter}, cost: ${
                giverBalanceBefore - giverBalanceAfter
            }`,
        )
    } finally {
        sdk.close()
