...
export async function touch(options: { value: number, debug: boolean }) {
    ...
    try {
        ...
        const touch = new Account(Touch, { client: sdk, signer: giver.account.signer })
        ...
        if (touchAccount.acc_type != 1) {
            ...
            const timestampOld = BigInt((await touch.runLocal("getTimestamp", {})).decoded?.output.value0 ?? 0)
            const balanceBefore = BigInt(await touch.getBalance() ?? 0)

            const response = await touch.run("touch", {})
            touch.refresh()

            const timestampNew = BigInt((await touch.runLocal("getTimestamp", {})).decoded?.output.value0 ?? 0)
            const balanceAfter = BigInt(await touch.getBalance() ?? 0)

            if (timestampNew > timestampOld) {
                console.log(`Touch ok: ${response.transaction.id}\nTouch balance: ${balanceAfter} cost: ${balanceBefore - balanceAfter}`)
            } else {
