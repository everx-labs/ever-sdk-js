...
test('Deploy from contract', async () => {
    const { contracts, crypto } = tests.client;
    const keys = await crypto.ed25519Keypair();
    ...
    const address = await contracts.run({
    ...
    expect(address.output.value0).toEqual(`0x${deployData.accountId}`);
    ...
    expect(result.output.value0).toEqual('0x0');
});
